...
Foyer Lobby is pre-1.0. Security fixes are applied to the latest published npm version only.

## Local Security Model

Foyer Lobby is a local developer tool. The server binds to `127.0.0.1` and exposes unauthenticated localhost routes for Claude Code and Codex hooks.
...
Provider credentials may be stored in `~/.config/foyer-lobby/config.env` when using the Anthropic API backend. Keep that file private.
