# Design System — Foyer · "Instrument"
...
- **What this is:** A local dashboard that turns Claude Code's 3–5 min agent waits into
  ...
  renders the current task, a live "what the agent is doing now" summary, and a
  deep-research panel.
...
Foyer is the _instrument cluster for your agent_: a warm-black control panel of
...
(click, configure, navigate) and converges on blue-on-cold-black. Foyer is the rare
...
| 2026-06-09 | The Focus / Research **view-tab strip is always shown** for an open session (was gated on having ≥1 briefing). The empty Research tab is a first-class, status-aware state: when topics exist it surfaces the suggested-topic **launcher chips inline**; otherwise an honest status message (generating spinner / "no topics yet" / "briefings open here"). No new amber — only the existing primed/warming chip dots carry it; the empty glyph stays `--text-dim`.                                                                                                                                                                                                                                                                                    | "Always show the tab" with a real empty state, not a hidden feature. The prior empty copy ("tap a topic in Deep Research") pointed at the Focus-rail launcher, which isn't rendered in the Research view — a dead-end CTA. Surfacing the same launcher inline makes the action live where the user is standing, so the always-on tab is never a dead control (it's a launcher or an honest readout), satisfying "readout or signal, never decoration." The launcher is extracted into a shared `ResearchChips` component (DRY) so the rail and the empty tab share one source of truth incl. the primed/warming dots.                                                                                                                              |
| 2026-06-09 | Removed the **Live Files** panel (`02 · TOUCH POINTS`) entirely — UI, the `touch` SSE event, server storage (`addTouchPoint`/`Session.touchPoints`), the `PostToolUse` touch recording, and the internal feed of recent file ops into the Current Focus summarizer. Research is renumbered `02`. The right rail now holds only the deep-research panel.                                                                                                                                                                                                                                                                                                                                                                                                | A value audit found the panel earns its pixels only _during_ a run (after, `git diff` dominates it) and its signal drowns in `Read`/`Bash` noise as sessions grow — it fails "every element is a readout or a signal, never decoration" once the firehose kicks in. The unique value (liveness + out-of-scope blast-radius) belongs in a future sharpened signal, not a literal log. File-operation context already lives in the transcript the summarizer reads, so the narration is unaffected. Mirrors the 2026-06-08 workflow-storyline removal. Touch points were part of ADR 0002's persisted aggregate; that ADR's persistence decision stands, the field is just dropped (old session files load fine — the now-unknown field is ignored). |
| 2026-06-10 | **PRIMARY BRIEFING strip** — the one recommended read renders as the integrated TOP BAND of `02 · DEEP RESEARCH` (a readout strip with a 3px left status rail + one hairline divider), NOT a bordered card nested in the bordered module. States: extractive (WATCHING/MATCHED rows, no LED) → warming (pulsing hollow amber LED) → ready (solid glowing amber LED + amber keycap) → read (hollow dim LED) → error (red LED + RETRY). The reason line is always the LLM's specific why-now (no static subtitle); citations are plain readout text, not controls; per-state tabular readouts (warming = live elapsed, ready = frozen time-to-ready, error = `failed ×N`); a recessive `NOT USEFUL` action-row control with a 5s inline UNDO. (ADR 0005) | Both cross-model design voices rejected the bordered-card treatment as a stacked-cards-in-a-module hard-rejection pattern; precedence-by-position + rail + type weight beats precedence-by-border, and a reflowing band reads calmer than a resizing floating box. The primary is the marquee of the Live Learning Briefing, so it must read as the module's primary channel, not a widget in a widget. Reuses `--working`/`--amber` pending the Instrument `--signal` swap.                                                                                                                                                                                                                                                                       |
| 2026-06-10 | **System rule: one glow source per module.** Only one element per module may carry the amber box-shadow "glow" (e.g. the primary strip's ready LED); follow-up chip dots stay small and glow-free.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | With the agent's ON-AIR bezel + working LED already amber, multiple glowing amber elements in one 380px module made "your briefing is ready" indistinguishable from "agent is running" at a glance — the exact signal the feature exists for. Capping glow to one source per module keeps amber a rare, legible signal without banning its sanctioned uses (keycap, small status dots). (ADR 0005)                                                                                                                                                                                                                                                                                                                                                 |
| 2026-06-10 | **System rule: an LED is always paired with its state word**, never colour/shape alone (e.g. the strip kicker reads "Primary Briefing · Ready/Warming/Read/Failed" beside its LED).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Extends the existing "status never by colour alone" rule to the new multi-state LED. Under `prefers-reduced-motion` the warming pulse stops, so warming vs read vs ready must still be unambiguous from text + shape; the always-present state word guarantees it. (ADR 0005)                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
