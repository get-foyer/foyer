#!/usr/bin/env tsx
/**
 * Foyer Lobby — interactive setup wizard.
 *
...
import { showLobbyBanner } from '../server/banner.js';
...
function currentCliCommand(): string[] {
  ...
  return entrypoint ? [process.execPath, entrypoint] : ['foyer'];
}
...
export async function runSetup() {
  await showLobbyBanner({ subtitle: 'Setup' });
  ...
  // --- Write config ---
  const envLines = [
    '# Foyer Lobby — generated by `foyer setup`',
    `FOYER_PORT=${port}`,
  ...
  if (hasCodex) {
    ...
    if (codexHooksInstalled) {
      ...
      await installCodexHooks(codexConfigPath, currentCliCommand());
    }
  }
