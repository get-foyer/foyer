#!/usr/bin/env tsx
/**
 * Foyer — interactive setup wizard.
 *
 ...
 * 6. Write user config and install hooks
 */
import { select, input, confirm } from '@inquirer/prompts';
import { execFile as _execFile } from 'child_process';
import { promisify } from 'util';
import { writeFile, access, copyFile, mkdir } from 'fs/promises';
import { dirname, join } from 'path';
import { homedir } from 'os';
import { pathToFileURL } from 'url';
import { installHooks, installCodexHooks, resolveCliEntrypoint } from '../server/install.js';
import { showFoyerBanner } from '../server/banner.js';
...
export async function runSetup() {
  await showFoyerBanner({ subtitle: 'Setup' });
  console.log('This wizard configures your LLM backend, Claude Code hooks, and dashboard port.');
  console.log('You can re-run `foyer setup` at any time to reconfigure.\n');
  ...
  const providerChoice = await select<ProviderChoice>({
    message: 'Which LLM backend should the dashboard use for activity summaries and research?',
    choices: [
      {
        value: 'codex',
  ...
  if (!isDetected) {
    ...
    const proceed = await confirm({
      message: `"${providerChoice}" was not auto-detected. Activity summaries and research may fail until it is configured. Continue anyway?`,
      default: false,
    });
    if (!proceed) {
      console.log('\nSetup cancelled.\n');
  ...
  if (providerChoice === 'anthropic-api') {
    ...
    const modelChoice = await select({
      message: 'Model (for activity summaries and research):',
      choices: [
        {
  ...
  try {
    ...
    console.log(`\n✓ Backed up existing config to ${envPath}.foyer-backup`);
  } catch {
  ...
  // --- Write config ---
  const envLines = [
    '# Foyer — generated by `foyer setup`',
    `FOYER_PORT=${port}`,
  ...
  if (hasCodex) {
    ...
    if (codexHooksInstalled) {
      ...
      await installCodexHooks(codexConfigPath, [process.execPath, resolveCliEntrypoint()]);
    }
  }
  ...
  console.log('\n✅ Setup complete!\n');
  if (codexHooksInstalled) {
    console.log('ℹ  Codex monitoring is active. Run `foyer uninstall` to remove Codex hooks.');
  }
  console.log('Next steps:');
  console.log('  1. Start the dashboard:');
  console.log('       foyer start');
  ...
  console.log('To uninstall hooks:  foyer uninstall');
  console.log('To reconfigure:      foyer setup\n');
}
